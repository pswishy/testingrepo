...
class PostViewController: UIViewController {
    ...
    private func deleteAllPosts(completion: @escaping () -> Void) {
        // Query for all posts
        ...
        query.find { result in
            ...
            case .success(let posts):
                ...
                // Call completion after all posts are deleted
                completion()
            case .failure(let error):
                print("Error querying posts: \(error.localizedDescription)")
                // Call completion immediately in case of failure
                completion()
            }
        }
    }
